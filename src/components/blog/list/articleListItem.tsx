...
export const ArticleListItem = ({ shape, info }: ArticleListItemProps) => {
  ...
  const boxThumbnailImageStyle: CSSProperties = {
    objectFit: "cover",
  ...
  const commentLikeStyle: CSSProperties = {
    display: "flex",
    alignContent: "center",
  };
  ...
  return (
    <>
      ...
      >
        <Image
          src={info.thumbnailUrl ? info.thumbnailUrl : defaultThumbnailImage}
          alt={"thumbnailImage"}
          width={360}
          height={160}
          style={boxThumbnailImageStyle}
        ></Image>
        <div style={firstLineStyle}>
          <Profile
            size="small"
            username={info.authorName}
            imgUrl={
              info.authorPhotoUrl ? info.authorPhotoUrl : defaultProfileImage
            }
          />
          <div style={createdAtStyle}>{info.createdAt}</div>
        </div>
        <div className={nanumSquare.className} style={titleStyle}>
          {info.title}
        </div>
        <div className={nanumSquare.className} style={aboutStyle}>
          {info.about}
        </div>
        <div className={aldrich.className} style={rateContainerStyle}>
          <div style={viewCountStyle}>
            <div>View &nbsp;</div>
            <div style={{ fontSize: "9px", paddingTop: "2px" }}>●</div>
            <div>&nbsp;&nbsp;{info.viewCount}</div>
          </div>
          <div style={commentLikeStyle}>
            <CiChat1 size="20px" />
            <div style={{ padding: "4px" }}>{info.commentCount}</div>
            <CiHeart size="20px" />
            <div style={{ padding: "4px" }}>{info.like}</div>
          </div>
        </div>
        <div style={categoryContainerStyle}>
          {info.categoryName.map((category, index) => {
            return (
